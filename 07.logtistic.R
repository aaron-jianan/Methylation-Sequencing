...
m_glm <- predict(model_glm,newdata = testdata)
confusionMatrix(reference=testdata$group,data=m_glm)
library(pROC)

probs_glm <- predict(model_glm, newdata = testdata, type = "prob")

auc_glm <- roc(testdata$group, probs_glm[,2])$auc
auc_glm
